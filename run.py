...
from pipelines import (
    e2e_ts_forecast_batch_inference,
    e2e_ts_forecast_deployment,
    e2e_ts_forecast_training,
)
...
Run the e2e_ts model training pipeline with various
options.
...
def main(
    ...
    if not only_inference:
        ...
        pipeline_args[
            "run_name"
        ] = f"e2e_ts_forecast_training_run_{dt.now().strftime('%Y_%m_%d_%H_%M_%S')}"
        e2e_ts_forecast_training.with_options(**pipeline_args)(**run_args_train)
    ...
    pipeline_args[
        "run_name"
    ] = f"e2e_ts_forecast_deployment_run_{dt.now().strftime('%Y_%m_%d_%H_%M_%S')}"
    e2e_ts_forecast_deployment.with_options(**pipeline_args)(**run_args_inference)
    ...
    pipeline_args[
        "run_name"
    ] = f"e2e_ts_forecast_batch_inference_run_{dt.now().strftime('%Y_%m_%d_%H_%M_%S')}"
    e2e_ts_forecast_batch_inference.with_options(**pipeline_args)(**run_args_inference)
